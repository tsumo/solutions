...
(defun dod-request-handler (path header params)
  (if (equal path "game.html")
      (progn (princ "HTTP/1.x 200 OK")
             (write-char #\return)
             (write-char #\linefeed)
             (write-char #\return)
             (write-char #\linefeed)
             (princ "<!DOCTYPE html>")
             (tag html () (tag head () ()) (tag body () (tag center ()
               ...
               (let ((chosen (assoc 'chosen params)))
                 ...
                 (cond ((lazy-null (caddr *cur-game-tree*))
                        (web-announcer-winner (cadr *cur-game-tree*)))
                       ((zerop (car *cur-game-tree*))
                        (web-handle-human
                          (when chosen (read-from-string (cdr chosen)))))
...
(defun web-announcer-winner (board)
  (fresh-line)
  (let ((w (winners board)))
    (if (> (length w) 1)
